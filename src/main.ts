...
async function bootstrap() {
          ...
          format: winston.format.combine(
            ...
            winston.format.printf(
              ({ timestamp, level, message, trace, context }) =>
                `${timestamp};${level};${message}${trace ? `;${trace}` : ''}${context ? `;${JSON.stringify(context)}` : ''}`,
            ),
          ),
        }),
      ],
  ...
  app.useGlobalGuards(new JwtAuthGuard(app.get(Reflector)), new RoleGuard(app.get(Reflector), app.get(AuthService)));
