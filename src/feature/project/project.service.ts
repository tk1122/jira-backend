...
export class ProjectService {
  ...
  async updateProject(
    ...
    if (leader) {
      if (leader.id !== oldLeader.id) {
        this.notifService
          .createNotifications(pm, [oldLeader], project.id, ProjectEntityType, project.name, NotifEventType.RemovedFromProject)
          .then();
        this.notifService
          .createNotifications(pm, [leader], project.id, ProjectEntityType, project.name, NotifEventType.AddedToProject)
          .then();
      }
    ...
    if (members) {
      ...
      this.notifService
        .createNotifications(pm, newMembers, project.id, ProjectEntityType, project.name, NotifEventType.AddedToProject)
        .then();

      const removedMembers = oldMembers.filter(m => !memberIds.includes(m.id));
      this.notifService
        .createNotifications(pm, removedMembers, project.id, ProjectEntityType, project.name, NotifEventType.RemovedFromProject)
        .then();
    ...
    if (isProjectUpdated) {
      ...
      this.notifService
        .createNotifications(pm, membersAndLeader, project.id, ProjectEntityType, project.name, NotifEventType.Updated)
        .then();
    }
  ...
  async deleteProject(projectId: number, userId: number) {
    ...
    this.notifService
      .createNotifications(pm, [leader, ...members], project.id, ProjectEntityType, project.name, NotifEventType.Deleted)
      .then();
