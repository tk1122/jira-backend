...
export class AuthService {
  ...
  async createSession(username: string, password: string): Promise<JWTPayload & { accessToken: string }> {
    ...
    if (!user) {
      this.logger.error(`Can not find user with username ${username}`, Error().stack, 'AuthService:createSession');
      throw new UnauthorizedException('Invalid username or password');
    }

    const isPasswordValid = await compare(password, user.password);
    if (!isPasswordValid) {
      this.logger.error(`Password is incorrect with ${username}`, Error().stack, 'AuthService:createSession');
      throw new UnauthorizedException('Invalid username or password');
    }
